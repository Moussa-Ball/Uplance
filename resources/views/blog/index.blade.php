                      ...
                      <a href="{{ route('blog.read', ['slug' => $article->slug, 'id' => $article->id]) }}" class="blog-post">
                          ...
                          <div class="blog-post-thumbnail">
                              <div class="blog-post-thumbnail-inner">
                                ...
                                @endforeach
                                  <img src="{{ Storage::url($article->image) }}" alt="image-{{ $key }}">
                              </div>
                          </div>
                          <!-- Blog Post Content -->
                          <div class="blog-post-content">
                              <span class="blog-post-date">{{ $article->created_at->format('d M Y') }}</span>
                              <h3>{{ $article->name }}</h3>
                              {!! (strlen($article->content) > 255) ? substr($article->content, 0, 255).'...' : $article->content !!}
                          </div>
                          <!-- Icon -->
                        ...
                        <form method="get" action="{{ route('blog.search') }}">
                            <div class="sidebar-widget margin-bottom-40">
                                <div class="input-with-icon">
                                    <input class="custom-search-input" name="q" value="{{ old('q', Request::get('q')) }}" id="autocomplete-input" type="text" placeholder="Search">
