    ...
    setUserPresence(state, user) {
        for (let i in state.conversations.threads) {
            if (state.conversations.threads[i].user.hashid == user.hashid) {
                state.conversations.threads[i].user.presence_status = user.presence_status;
                state.conversations.threads[i].user.switcher_status = user.switcher_status;
            }
        }
    },
...
const actions = {
    getConversations(context) {
        ...
        this._vm.axios.get('/api/messages/conversations')
            .then(response => {
                conversations = response.data
                context.commit('addConversations', conversations)
                Echo.private(`App.User.${response.data.owner}`)
                    .listen('NewMessage', (e) => {
    ...
    async getMessages(context, threadId) {
        let response = null
        await this._vm.axios.get(`/api/messages/discussions/${threadId}`)
            .then(response => {
                context.commit('addMessages', response.data)
            }).catch(error => { this._vm.showErrors(error) })
    },
    sendMessage(context, { threadId, content }) {
        this._vm.axios.post(`/api/messages/store/${threadId}`, { 'content': content })
            .then(response => {
    ...
    activeThread(context, threadId) {
        this._vm.axios.put('/api/messages/active/' + threadId, {})
            .then(response => {
    ...
    markAsRead(context, threadId) {
        this._vm.axios.put(`/api/messages/mark-as-read/${threadId}`, {})
            .then(response => {
    ...
    loadPreviousMessages(context, threadId) {
        ...
        if (message) {
            let url = '/api/messages/previous/' + threadId + '?before=' + message.created_at
